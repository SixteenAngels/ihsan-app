-- Live Chat System Database Schema (CORRECTED)
-- This extends the existing Ihsan database with chat functionality
-- Uses correct user roles: customer, vendor, vendor_manager, admin
...
-- RLS Policies for chat_rooms (CORRECTED - using correct roles)
CREATE POLICY "chat_rooms_select_policy" ON chat_rooms
    FOR SELECT USING (
        ...
        EXISTS (
            ...
            AND profiles.role IN ('admin', 'vendor_manager')
        )
    );
...
-- RLS Policies for chat_messages (CORRECTED)
...
-- RLS Policies for chat_participants (CORRECTED)
...
-- RLS Policies for chat_typing (CORRECTED)
...
-- RLS Policies for chat_assignments (CORRECTED)
